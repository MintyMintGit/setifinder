...
//        // breadcrumb links
    ...
    public function enqueue_scripts() {
        ...
        wp_enqueue_script( 'listify-app-listing', get_template_directory_uri() . '/inc/integrations/wp-job-manager/js/listing/app.min.js', array( 'listify-app-map' ), 20161114 );
    ...
    public function job_listing_class( $data, $json = false ) {
		...
		$classes = array(
			...
			'hentry',
			'card-style--' . Listify_WP_Job_Manager_Template_Card::get_style()
		);
...
<?php
    ...
    public function the_location() {
		...
		$location = $this->get_the_location();
		if( ! $location ){
			return;
		}
?>

<div class="job_listing-location job_listing-location-<?php echo esc_attr( $format ); ?>" itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
	<?php echo $location; ?>
</div>
...
<?php
	...
	/**
	 * Get the location and format depending on the customizer setting.
	 *
	 * @since 1.0.0
	 *
	 * @return string $location
	 */
    public function get_the_location( $format = false ) {
		...
		if ( 'none' == $format ) {
			return $location;
		} else if ( 'formatted' == $format ) {
			return $this->get_the_formatted_location( $location );
		} else if ( 'coordinates' == $format ) {
			return $this->get_the_coordinates();
		}
    }
    ...
    public function get_the_formatted_location( $location = false ) {
        ...
        $output[] = sprintf( '<a class="google_map_link" href="%s" target="_blank">', $this->google_maps_url() );
        $output[] = wc()->countries->get_formatted_address( $address );
        $output[] = '</a>';
	...
	/**
	 * Get the coordinates for a listing.
	 *
	 * @since 1.8.0
	 *
	 * @return string $coordinates
	 */
	public function get_the_coordinates( $listing = false ) {
		$post = get_post();
    ...
    public function google_maps_url() {
        ...
        $args = array(
            'daddr' => urlencode( $this->get_the_coordinates() )
        );
...
<div class="content-single-job_listing-title-category">
    ...
    public function the_directions() {
		...
		$address_format = get_theme_mod( 'listing-address-format', 'formatted' );

		if( 'formatted' == $address_format ) {
			$address_format = 'none';
		}

		$destination = $this->get_the_location( $address_format );
		if( !$destination ){
			return;
		}
?>
...
<?php
    ...
    ?>
        <div class="archive-job_listing-filter-title">
            ...
            <?php do_action( 'archive_job_listing-results-title-before' ); ?>

			<?php if ( apply_filters( 'listify_listings_display_style_switcher', false ) ) : ?>
            ...
            <?php do_action( 'archive_job_listing-results-title-after' ); ?>
        </div>
    <?php
    }
