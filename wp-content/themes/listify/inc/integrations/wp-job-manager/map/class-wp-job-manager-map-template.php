...
class Listify_WP_Job_Manager_Map_Template extends listify_Integration {
	...
	public function listing_data( $data ) {
		...
		/**
		 * If the popup link should open in a new window/tab.
		 *
		 * @see `listing-archive-window` customize control.
		 */
		$data[ 'target' ] = get_theme_mod( 'listing-archive-window', false ) ? '_blank' : '';
	...
	public function enqueue_scripts( $atts ) {
		...
		$settings = array(
			...
			'trigger' => wp_is_mobile() ? 'click' : listify_theme_mod( 'map-behavior-trigger', 'mouseover' ),
	...
	public function register_scripts() {
		...
		wp_register_script( 'listify-app-map', Listify_Integration::get_url() . 'js/map/app.min.js', $deps, 20161219, true );
	}
		...
		<div class="search-radius-wrapper in-use">
			<div class="search-radius-label">
				<label for="use_search_radius">
					...
					<?php printf( __( 'Radius: <span class="radi">1</span> mi', 'listify' ), $this->get_average_radius(), $this->unit() ); ?>
				</label>
			</div>
